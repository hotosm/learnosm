...
A simple, light-weight, and minimalistic app for recording GPS traces on the Android platform. The easy-to-use interface with the sole purpose of GPS logging makes for a very battery-efficient application that can be saved in GPX, KML, or text file formats. Log files can be uploaded to OpenStreetMap, an OpenGTS server, DropBox or sent to an email addresss.
...
Features
--------------------------
* Specify time or distance intervals 
* Select Cellphone tower or GPS satellites as 
* Log to GPX, KML, CSV, TXT or NMEA files
...
* Google Drive - upload GPX/KML/ZIP/TXT
...
The **Views drop-down** lets you to choose how information is display on the canvass.
...
The **Satellites** icon will show you how many satellites you're fixed on.
...
The **Status** indicator will display in green when it is recording, while a spinner is displayed when the device is tring to get a fix.
...
The **Bearing** tells you the relative direction you're moving on.

The **Speed** will give you approximate information of how fast you're going.
...
The **General options** is where to find settings for *Startup*, *unit of measurement* (Metric or Imperial), *debug file* and *version information*.

![Menus1][]


The **Logging details** is where to find settings  *File formats*, *folder* path to store logs, rules on *new file creation*, the  *frequency* , *accuracy*.
...
When you click the *Authorize this app*, you will be directed to your Internet browser and to the OpenStreetMap website. If you're not logged in, you will be asked for your credentials.
...
To start collecting logs (in Simple view), simply click on the blue button. A spinner will appear near the upper right corner of the screen to indicate the atttempt to get a fix on satellites. A green button will a fix is made, and recording is on going.
...
It's also possible to directly call the *Annotate* button from Android's Notification bar. Select the app from the Task List and click on the *Annotate* button.
...
#####By time
**Time before logging** is set to 60 seconds on default. You can change this to five or ten seconds, when you're walking instead of riding a vehicle. When you're in a car, you can set this to 1 second to generate very accurate logs.
...
​Sharing traces 
---------------
You may also share the recorded traces, or your current location to other people. Sharing options may vary from one device to another, or the apps installed in a device. Below is an example of how a sharing option screen may look like:
...
Display Options
-----------------
The canvass can be rendered differently, whichever you find more appropriate to your taste:
