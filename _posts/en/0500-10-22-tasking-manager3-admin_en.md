...
**iD Editor Presets** - The HOT Tasking Manager now has preset restrictions, where the project manager can limit what mappers can map in the iD Editor. For example, on a buildings-only project you can set it so only `building=yes` appears in iD. Advanced mappers can still add other tags manually.

This new feature was added to reduce the amount of extraneous and incorrect data coming from Tasking Manager projects. New mappers will often add extraneous and incorrect features, such as inaccurate land use and poorly classified highways which do not fit in with the local road network.

To take advantage of these preset restrictions, `Expert Mode` needs to be turned on under your Tasking Manager user profile.

![TM Expert Mode][]

In the metadata section of your project, you'll need to define the the  **Type of Mapping** you want for your project. The iD presets will be limited to these features.

![TM Select Type Presets][]

Next you’ll choose what presets you want to restrict mapping to,such as just `building=*`, or `building=house`, or `highway=residential`. You can also allow multiple tags, such as `highway=residential`, `highway=unclassified`, and `highway=track`.

Example of buildings only:

![TM Buildings Preset][]

When enabled on the Tasking Manager, the mapper will only see the presets you have chosen, such as building in this case. If a user tries to map something else, like a line feature, no presets will appear.

![TM Buildings Only][]

Note: it is recommended that you add something to the project instructions about the preset, for example: "This project will allow for mapping buildings only. Limited presets will be available in the iD editor."
...
[TM Expert Mode]: /images/coordination/tm3_expert_mode.png
[TM Select Type Presets]: /images/coordination/tm3_type_presets.png
[TM Buildings Preset]: /images/coordination/tm3_buildings_preset.png
[TM Buildings Only]: /images/coordination/tm3_buildings_only.png
