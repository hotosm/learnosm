...
En esta sección vamos a ver el Gestor de Tareas, una herramienta intuitiva que los mapeadores pueden utilizar para ordenar un área mediante una cuadrícula, y poder trabajar de forma conjunta mapeando el área de forma organizada.

El Gestor de Tareas de OSM permite a los mapeadores de todo el mundo ayudar mapeando una región con un mínimo riesgo de que se solapen las zonas de trabajo.

Esto permite que la gente sobre el terreno y la gente trabajando remotamente (llamados a veces 'mapeadores de sillón') colaboren de manera efectiva, rápida y evitando que sea necesario rehacer el trabajo debido a conflictos.  


## Vista general del proceso
1. Un Administrador elige un área  en OpenStreetMap que necesita ser actualizada. El administrador se asegura de que haya imágenes satélite de fondo sobre las que los mapeadores puedan calcar, y crea un proyecto que cubra ese área. Hay que especificar el nivel de detalle que se necesita, así como la urgencia y otras informaciones que necesitarán los mapeadores. Una vez completado, el Administrador publica el proyecto en el Gestor de Tareas, si bien es posible editar y hacer cambios con posterioridad, si fuera necesario.

2. Un mapeador remoto selecciona la tarea de un cuadrado, completa el mapeado, y marca el cuadrado como completo. 

3. Un segundo mapeador remoto comprueba que el cuadrado está completo a un nivel satisfactorio y marca el cuadrado como validado.

4. El progreso del proyecto se puede monitorizar desde la pestaña "stats" del proyecto, y el proyecto se puede bajar de nivel o archivar si así lo considera el Administrador.
...
Para obtener información acerca del Gestor de Tareas, los sponsors de HOT, 
For information about the Gestor de Tareas, HOT sponsors, partners & help click on **About** at the top of the page.

![Gestor de Tareas About][]

Una vez conectado con su nombre de usuario, puede pulsar sobre el nombre de usuario que aparece en la parte superior. Desde ahí podrá:

- Acceder a la lista de proyectos en los que ha completado cuadrados.
- Ir a la pantalla de sus **Messages** dentro del Gestor de Tareas (no es la misma que la del sistema de mensajes de OSM) 
- Desconectar
...
Todo lo que se necesita conocer acerca del proyecto se encuantra aquí. A la izquierda hay una descripción del proyecto de mapeo y qué es lo que se necesita mapear. A la derecha está la cuadrícula con el área a mapear.

- La línea malva en los cuadrados del perímetro de un proyecto indica el límite del proyecto. Esta línea suele seguir las imágenes de fondo, por lo que puede tener una forma inhabitual. A pesar de que es posible mapear fuera de ese perímetro, no es necesario y los validadores no tendrán en cuenta ese trabajo.

- Las zonas rosas del mapa indican las zonas prioritarias a mapear.
...
Pulse cuando esté preparado para empezar a mapear. Se puede seleccionar el cuadrado para mapear, o seleccionando directamente un cuadrado en el mapa, o pulsando en el botón "Tarea aleatoria" ("Take a Task at random").

![Picking a task][]

## Seleccionar un cuadrado para mapear
![Assigned task square][]  
Una vez seleccionado un cuadrado se puede mirar si tiene alguna historia, como puede ser algún mapeador que ya ha comenzado con ese cuadrado, pero que se dio cuenta de que no podía acabarlo.
...
Pulsando el botón **Start Mapping** se bloquea el cuadrado de forma que ningún otro mapeador pueda seleccionarlo hasta que el cuadrado sea liberado, iniciándose además un contador de tiempo de dos horas ( 120 minutos), al final del cual el cuadrado se liberará automáticamente.

Es una buena práctica comprobar regularmente el contador de tiempo, es fácil enfrascarse en el trabajo y no darse uno cuenta de que el cuadrado ha sido liberado y otro mapeador lo ha  seleccionado y ha comenzado a mapear en él. Esto puede ser una fuente de conflictos y problemas.
...
- Para utilizar únicamente cuando se esté involucrado en un proyecto en el que un mapeador local ha realizado una toma de datos sobre el terreno anotando la información recogida en los mapas papel. Este mapa puede escanearse y utilizarse como imagen de fondo por parte de un mapeador remoto para leer la información y actualizar los datos en OpenStreetMap.

### Subdividiendo un cuadrado (Splitting a square)  
Una vez seleccionado un cuadrado e inspeccionado con las imágenes de fondo, puede suceder que requiera mucho trabajo de detalle para mapearlo. Un ejemplo podría ser trazar los edificios en áreas urbanas densas, o localizar pequeñas poblaciones en áreas muy extensas.
 ...
 * Evite mapear fuera de su cuadrado, otros mapeadores podrían estar trabajando en esos cuadrados, dando lugar a duplicación de esfuerzos. Es correcto mapear objetos que están sobre el borde, por ejemplo un edificio, pero no vaya más allá.
 * Extienda las carreteras, ríos u otros elementos lineales un poco más allá del borde, así el siguiente mapeador podrá extenderlos desde donde los deje usted.
 * Si tiene dudas acerca de un elemento concreto, use la sección de comentarios para preguntar o eche un vistazo al wiki.
 * Si comete un error importante, por ejemplo borrar un elemento de primer orden o una relación, utilice la caja de comentarios para pedir ayuda a otros mapeadores acerca de como revertir la situación. Procure incluir el conjunto de cambios (changeset) o una descripción de qué ha ocurrido. Al tratarse de un trabajo colaborativo, hay muchos otros mapeadores para ayudar. Es importante tener en cuenta que todo el mundo comete errores alguna vez.
 * No dude en pedir consejo, la mapeadores que validan su trabajo pueden ser bruscos o directos, pero sabiéndolo, es correcto establecer un diálogo con ellos, el resultado será positivo para todas las partes.
 * No debe validar usted mismo su propio trabajo, otro par de ojos conduce siempre a una mejor calidad del mapa resultado.
 * No se preocupe si otros mapeadores son bruscos al validar su trabajo. Igual que usted, lo único que quieren es asegurarse de que todos los datos se mapean adecuadamente. El feedback se refiere siempre al trabajo pendiente, no es en absoluto una crítica al trabajo que ha realizado.
