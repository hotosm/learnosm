...
1. Individua l'area che desideri mappare sul sito web Filed Papers. Stampa una mappa di questa zona. Puoi scegliere di stampare la mappa corrente dell'area su OpenStreetMap, oppure puoi scegliere di stampare immagini aeree, se è disponibile quella della tua zona.
![field papers homepage][]
2. Usa la stampata della tua mappa per analizzare la zona. Aggiungere altri luoghi
disegnandoli sulla mappa. Disegna linee per le strade, forme per edifici, e così via. Scrivi note su ogni posizione direttamente sulla mappa, o scrivi i numeri sulla mappa che si riferiscono ai numeri nei tuoi appunti, dove  puoi scrivere informazioni più dettagliate su ogni oggetto.

![paper example][]
3. Effettua la scansione del documento e importalo nel computer. Se non disponi di uno scanner, puoi fotografare il foglio, se la tua macchina fotografica è in grado di scattare foto di alta qualità. Carica l'immagine nel sito web di Field Papers.
4. Carica i Field Papers su JOSM. Usa gli oggetti disegnati come riferimento per aggiungerli digitalmente in OpenStreetMap.
...
Ora impariamo come creare e utilizzare i Field Papers.
...
Se la prima volta fai degli errori, non preoccuparti, è possibile creare tutte le Field Paper di cui hai bisogno.
...
-  Quando la stampa è pronta, clicca su "Scarica mappa PDF per la stampa". Field paper dovrebbe iniziare il download.
...
---------------
I Field Papers sono molto utili per la mappatura con la sola carta, ma non sono magici. Dovrai ancora inserire la carta in JOSM, aggiungere le informazioni in formato digitale, e salvare le modifiche su OpenStreetMap.
...
- Aprire JOSM e vai in Modifica -> Preferenze. 
...
Aprili in JOSM
------------
 Ora puoi caricare il tuo Field Paper su JOSM e usarlo per aggiungere le informazioni raccolte su OpenStreetMap. Ritorna a sito web di Field Papers, digitando nel browser, come già fatto prima, [fieldpapers.org] (http://fieldpapers.org/). 
...
- Per caricare il Field Paper su JOSM, devi copiare la snapshot dell'ID del Field Papers scannerizzati. Nella barra delle Url nella parte superiore del browser internet, selezionate il testo e premere CTRL + C sulla tastiera per copiare. Il testo dovrebbe essere simile a questo: 
